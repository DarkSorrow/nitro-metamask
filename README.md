...
- [`android/`](android): All your `android`-specific implementations.
  - [`build.gradle`](android/build.gradle): The gradle build file. This contains four important pieces:
    ...
    3. Adds all Nitrogen files (`apply from: '.../NitroMetamask+autolinking.gradle'`)
  ...
  - [`CMakeLists.txt`](android/CMakeLists.txt): The CMake build file to build C++ code. This contains four important pieces:
    1. Creates a library called `NitroMetamask` (same as in `nitro.json`)
    2. Adds all Nitrogen files (`include(.../NitroMetamask+autolinking.cmake)`)
  ...
  - [`src/main/java/com/margelo/nitro/nitrometamask/`](android/src/main/java/com/margelo/nitro/nitrometamask/): All Kotlin implementations.
    - [`NitroMetamaskPackage.kt`](android/src/main/java/com/margelo/nitro/nitrometamask/NitroMetamaskPackage.kt): The react-native package. You need this because the react-native CLI only adds libraries if they have a `*Package.kt` file. In here, you can autolink all Kotlin HybridObjects.
...
- [`NitroMetamask.podspec`](NitroMetamask.podspec): The iOS podspec build file to build the iOS code. This contains three important pieces:
